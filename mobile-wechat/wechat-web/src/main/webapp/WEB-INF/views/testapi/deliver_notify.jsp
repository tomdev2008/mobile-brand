...
<script type="text/javascript">
	...
	$(function(){
		$("#out_trade_no").val("112342343");
		$("#openid").val("oKr0st86VMPwWMdi10CJLEXkdskI");
		$("#transid").val("1217962501201401148124199999");
	...
	function deliverNotify(){
		$.ajax({
			type : 'POST',
			dataType:'json',
			url :"<c:url value='/testapi/deliverNotify'/>",
			data : {
...
<body>
	<div id="w_login" class="wrapper">
		...
		<fieldset>
			<br>
			<legend ><span style="font-size:20px;color:red">微信API接口测试 ==> 发货通知接口</span></legend>
			<p>
				<label for="email"><span style="font-size:20px;color:red">订单号：</span></label> 
				<input id="out_trade_no" name="out_trade_no" type="text" height="50px" required style="font-size:20px;">
			</p>
			<p>
				<label for="email"><span style="font-size:20px;color:red">第三方流水号：</span></label> 
				<input id="transid" name="transid" type="text" height="50px" required style="font-size:20px;">
			</p>
			<p>
				<label for="email"><span style="font-size:20px;color:red">微信ID：</span></label> 
				<input id="openid" name="openid" type="text" height="50px" required style="font-size:20px;">
			</p>
			<br>
			<input id="query_btn" type="button" class="login_btn" value="订单查询" /> <br>
		</fieldset>
		<table>
			<tr>
