...
<form id="mainForm" name="mainForm" action="${rc.getContextPath()}/userManager/viewUserFans" method="GET">
...
<div class="adminMain_wrap">
	...
	<div class="adminContent clearfix">
		<table width="100%" class="table_bg01 table_hg01">
			<thead>
				<tr>
					<th width="15%">成为粉丝时间</th>
					<th width="15%">粉丝</th>
					<th width="70%"></th>
				</tr>
			</thead>
			<tbody>
			<#if (fansList?? && fansList?size > 0)>
			<#list fansList as fans>
			<tr>
			    <td><#if fans.CREATE_DATE??>${fans.CREATE_DATE?string('yyyy-MM-dd HH:mm:ss')}</#if></td>
			    <td><a href="javascript:void(0);" onclick="viewShowUser('${fans.USER_ID!''}');" title="${fans.PET_NAME!''}">${fans.PET_NAME!''}</a></td>
			<tr>
		    </#list>
		    <#else>
