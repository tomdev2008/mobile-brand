...
<table class="adminMain_top">
	<tbody>
		<tr>
			<td class="td01"></td>
			<td class="td02">
			<h3 class="topTitle fb f14">秀客行为统计</h3>
			</td>
			<td class="td03"></td>
		</tr>
	</tbody>
...
<form id="mainForm" name="mainForm" action="${rc.getContextPath()}/showStatistics/userBehavior" method="GET">
	...
	<div class="adminContent clearfix">
		<table width="100%" class="table_bg01 table_hg01">
			...
			<tr>
			    ...
			    <td><#if statistics.PUBLISH_SHOW_NUM??><a href="javascript:void(0);" onclick="viewShowStatistics('${statistics.TIMESS!''}');" title="发秀数">${statistics.PUBLISH_SHOW_NUM!''}</a><#else>0</#if></td>
			    ...
			    <td><#if statistics.SHARE_NUM??>${statistics.SHARE_NUM}<#else>0</#if></td>
			</tr>
		    </#list>
		    <#else>
...
<script type="text/javascript">
	...
	function exportData(){
		...
		var beginTime = $("#beginTime").val();
		var endTime = $("#endTime").val();
		location.href = "${rc.getContextPath()}/showStatistics/userBehaviorExport?timeType="+timeType+"&userType="+userType+"&beginTime="+beginTime+"&endTime="+endTime;
	}
	
	//查询秀统计
	function viewShowStatistics(times) {
		var beginTime = $("#beginTime").val();
		var endTime = $("#endTime").val();
		if(times != '总计') {
		...
		var pageUrl = "${rc.getContextPath()}/showStatistics/showBehavior?praisedOrder=1"+"&beginTime="+beginTime+"&endTime="+endTime;
		window.open(pageUrl);
	}
