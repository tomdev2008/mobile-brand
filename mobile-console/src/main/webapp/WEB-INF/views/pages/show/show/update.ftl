...
<table class="adminMain_top">
	<tbody>
		<tr>
			<td class="td01"></td>
			<td class="td02">
			<h3 class="topTitle fb f14">编辑秀</h3>
			</td>
			<td class="td03"></td>
		</tr>
	</tbody>
...
<div class="adminMain_wrap">
	...
	<!--导航end-->
<form id="addShowForm" name="addShowForm" method="post" action="${rc.getContextPath()}/show/update">
	<!--菜单内容-->
		...
		<table width="100%" class="table_bg05">	
			...
			<tr>
				...
				<td class="tdBox">
					<ul id="img_ul" class="ui-sortable">
						<#list show.pictureList as pic>
						<li class="img_li">
							<img onclick="deleteImg(this);" src="/resources/manager/images/p_t_exit.gif">
							<img width="260px;" height="270px;" src="${pic.newThumbnailUrl!''}" onclick="addGoodSn(event,this);">
							<input id="pictures" type="hidden" name="pictures" value="${pic.originalPicUrl!''}">
							<input id="ratioTypes" type="hidden" name="ratioTypes" value="${pic.ratioType!''}">
							<#list pic.tagList as tag>
								<div id="dragDivss" style="position: absolute; left: ${tag.xPosition!''}px; top: ${tag.yPosition!''}px; border: medium solid black;">
								<div onmousedown="drag (this.parentNode,event);" ondblclick="deleteTag(this)" style="font-family:sans-serif;font-weight:bold;background-color: #000000;color: #ffffff;"><#if tag.name?length lt 20>${tag.name!''}<#else>${tag.name[0..20]}</#if></div>
								<input id="tagTypes" type="hidden" value="${tag.type!''}" name="tagTypes">
								<input id="tagNames" type="hidden" value="${tag.name!''}" name="tagNames">
								<input id="objectId" type="hidden" value="${tag.objectId!''}" name="objectId">
								<input id="zyPosition" type="hidden" value="${tag.top!''}">
								<input id="zxPosition" type="hidden" value="${tag.left!''}">
								</div>
							</#list>
						</li>
						</#list>
			...
			<tr>
				...
				<td class="tdBox" id="goodsNameTd">
					<#list show.pictureList as pic>
						<#list pic.tagList as tag>
							<#if tag.type??&&tag.type==3>
								<input type="text" id="goodsName" name="goodsName" style="width: 500px;height:25px;" value="${tag.name!''}" />
							</#if>
						</#list>
					</#list>
				</td>
			...
			<tr>
				...
				<td class="tdBox">
				<textarea id="content" style="width:500px;height:80px;" name="content" maxlength="500">${show.content!''}</textarea>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="tdBox">
                      <input name="orderSeq" id="orderSeq" value="${show.orderSeq!''}"/>
				</td>
			...
			<tr>
				...
				<td class="tdBox">
                       <input name="publishTime" value="${show.publishTime!''}" type="text" id="publishTime" value=""  maxlength="20" style="width:120px;"  onfocus="WdatePicker({skin:'default',dateFmt:'yyyy-MM-dd HH:mm:ss'})"  />
				</td>
			</tr>
			<tr>
...
<div id="uploadImgDiv" class="showbox"> 
    <form action="${rc.getContextPath()}/show/uploadPicture" enctype="multipart/form-data" method="post" name="uploadproofFrm" id="uploadproofFrm"  theme="simple">
		...
		<div class="btnsdiv">
		<!--<a herf="javascript:void(0);" class="userOperBtn" onclick="one();">1:1</a>
		<a herf="javascript:void(0);" class="userOperBtn" onclick="three();">3:4</a>-->
