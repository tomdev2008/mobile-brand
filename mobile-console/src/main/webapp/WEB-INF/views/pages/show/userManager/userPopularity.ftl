...
<form id="mainForm" name="mainForm" action="${rc.getContextPath()}/userManager/viewUserPopularity" method="GET">
		...
		<table width="100%" class="table_bg01 table_hg01">
			<thead>
				<tr>
					...
					<th width="15%">原因</th>
					<th width="15%">人气变更</th>
					<th width="15%">变更后人气</th>
					<th width="40%"></th>
				</tr>
			</thead>
			<tbody>
			<#if (popularityList?? && popularityList?size > 0)>
			<#list popularityList as popularity>
			<tr>
			    <td><#if popularity.createDate??>${popularity.createDate?string('yyyy-MM-dd HH:mm:ss')}</#if></td>
			    <td>${popularity.name!''}</td>
			    <td>${popularity.value!''}</td>
