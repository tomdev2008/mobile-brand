...
<table class="adminMain_top">
	<tbody>
		<tr>
			<td class="td01"></td>
			<td class="td02">
			<h3 class="topTitle fb f14">专题修改</h3>
			</td>
			<td class="td03"></td>
		</tr>
	</tbody>
...
<div class="adminMain_wrap">
	<div class="adminUp_wrap">
		...
		<dl class="adminPath clearfix">
			...
			<dd class="last"><h3>专题修改</h3>
			</dd>
		</dl>
	</div>
	<!--导航end-->
<form id="addSubjectForm" name="addSubjectForm" enctype="multipart/form-data" method="post" action="${rc.getContextPath()}/subject/update">
	...
	<div class="adminContent clearfix">
		<table width="100%" class="table_bg05">
			...
			<tr>
				...
				<td class="tdBox">
					  <input  name="name" id="name" value="${subject.name!''}"/> 
				</td>
			</tr>
			<tr>
				<th class="thList" scope="row">标题：</th>
				<td class="tdBox">
					  <input  name="title" id="title" value="${subject.title!''}"/> 
				</td>
			</tr>
			<tr>
				<th class="thList" scope="row"><font class="red">*</font>类型：</th>
				<td class="tdBox">
	 					<input type="radio" name="displayStytle" class="displayStytle" value="1" <#if subject.displayStytle==1>   checked="checked"</#if> />竖专题(大)
	 					<input type="radio" name="displayStytle" class="displayStytle" value="2"  <#if subject.displayStytle==2>   checked="checked"</#if>   />横专题(小)</td> 
			...
			<tr>
				...
				<td class="tdBox">
                      <input  name="orderSeq" value="${subject.orderSeq!''}" id="orderSeq"/> 			<font class="gray">按该值正序排序,值小在前</font>			
                       <div id="timemsg"></div>		
               </td>
			</tr>
			<tr>
				...
				<td class="tdBox">
				      <img src="${subject.outPic!''}" width="300px"/>
			...
			<tr>
				...
				<td class="tdBox">
				<input name="beginTime" value="${subject.startTime?string('yyyy-MM-dd HH:mm:ss')}" type="text" id="beginTime" value=""  maxlength="11" style="width:120px;"  onfocus="WdatePicker({skin:'default',dateFmt:'yyyy-MM-dd HH:mm:ss'})"  />
				</td>
			</tr>
			<tr>
				<th class="thList" scope="row"><font class="red">*</font>结束时间：</th>
				<td class="tdBox">
					<input name="endTime"  value="${subject.endTime?string('yyyy-MM-dd HH:mm:ss')}"  type="text" id="endTime" value=""  maxlength="11" style="width:120px;"  onfocus="WdatePicker({skin:'default',dateFmt:'yyyy-MM-dd 23:59:59'})"  />
				</td>
			</tr>
		    <tr>
			...
			<tr>
				...
				<div id="contentDiv" style="  width: 806px;height: 600px;border: 1px solid #ddd;float:left; overflow: scroll;">
				  			<#list subject.contentItemList as subjectItem>
				  			   <div class="content_item">
					  			    <#if subjectItem.contentItemType==1>
						  			   <input type="hidden" name="subjectItemType" value="1" />
						  			   <input type="hidden" name="subjectItem" class="subjectItem" value="" />
						  			   <textarea class="content_text" style="overflow-y: hidden; height: 35px;"
						  			    onpropertychange="this.style.height = this.scrollHeight+ 'px';" 
						  			    oninput="this.style.height = this.scrollHeight + 'px';"><#noescape>${subjectItem.contentItemData!''}</#noescape></textarea>
						  			    <select  name="urlType" id="status" style="width:120px;margin-left:8px;margin-right:8px;float: left;">
						  			    	<#if (subjectItem.type)??>
						  			    		<option value="URL" <#if subjectItem.type=='url'> selected="checked"</#if>>URL</option>
								  			    <option value="卖场" <#if subjectItem.type=='sales'>   selected="checked"</#if>>卖场</option>
								  			    <option value="卖场集" <#if subjectItem.type=='store'>   selected="checked"</#if>>卖场集</option>
								  			    <option value="商品分类" <#if subjectItem.type=='goodCommodity'> selected="checked"</#if>>商品分类</option>
								  			    <option value="品牌" <#if subjectItem.type=='brand'>   selected="checked"</#if>>品牌</option>
								  			    <option value="话题" <#if subjectItem.type=='topic'>   selected="checked"</#if>>话题</option>
								  			    <option value="秀" <#if subjectItem.type=='show'>   selected="checked"</#if>>秀</option>
								  			    <option value="商品" <#if subjectItem.type=='good'>   selected="checked"</#if>>商品</option>
								  			    <option value="专题" <#if subjectItem.type=='subject'>   selected="checked"</#if>>专题</option>
						  			    	<#else>
								  			    <option value="URL">URL</option>
								  			    <option value="卖场">卖场</option>
								  			    <option value="卖场集" >卖场集</option>
						  			    ...
						  			    <input name="subjectUrl" value="${subjectItem.linkType!' '}" style="height:20px;float: left;"/>
					  			   <#elseif  subjectItem.contentItemType==2>
					  			      <input type="hidden" name="subjectItemType" value="2">
					  			      <input type="hidden" name="subjectItem" value="${subjectItem.contentItemData!''}?param1=${subjectItem.param1!''}&param2=${subjectItem.param2!''}">
					  			       <img name="content_item" class="content_img" src="${subjectItem.contentItemImg!''}"></img>
					  			       <select  name="urlType" id="status" style="width:120px;margin-left:8px;margin-right:8px;float: left;">
							  			    <#if (subjectItem.type)??>
								  			    <option value="URL" <#if subjectItem.type=='url'>selected="checked"</#if>>URL</option>
								  			    <option value="卖场" <#if subjectItem.type=='sales'>selected="checked"</#if>>卖场</option>
						  			    ...
						  			    <input name="subjectUrl" value="${subjectItem.linkType!' '}" style="height:20px;float: left;"/>
					  			   <#elseif  subjectItem.contentItemType==3>
					  			          <input type="hidden" name="subjectItemType" value="3">
					  			          <input type="hidden" name="subjectItem" value="${subjectItem.contentItemData!''}">
					  			           <img name="content_item" class="content_img" src="${subjectItem.goodsImage!''}"></img>
					  			           <select  name="urlType" id="status" style="width:120px;margin-left:8px;margin-right:8px;float: left;">
							  			    <#if (subjectItem.type)??>
						  			    		<option value="URL" <#if subjectItem.type=='url'>   selected="checked"</#if>>URL</option>
								  			    <option value="卖场" <#if subjectItem.type=='sales'>   selected="checked"</#if>>卖场</option>
					  			   ...
					  			   <div class="content_item_but">
					  			      <input type="button" class="upBtn itemBtn userOperBtn" onclick="upSubjectItem(this)" value="上移">
					  			      <input type="button" onclick="downSubjectItem(this)" class="downBtn itemBtn userOperBtn" value="下移"> 
					  			      <input type="button" onclick="delSubjectItem(this)" class=" userOperBtn" value="删除"></input>
					  			      <input type="button" class="userOperBtn insertBtn" onclick="addSubject(this)" value="插入"/>
					  			      <input type="button" class="addBtn userOperBtn" onclick="addBoxText(this)" value="新增文字"/>
					  			      <input class="addBtn userOperBtn" value="新增图片" type="button" onclick="addBoxImg(this)"/>
					  			      <input value="新增商品" class="addBtn userOperBtn " type="button" onclick="addBoxGood(this)"/>
					  			   </div>
				  			   </div>
				  			</#list>
				</div>
				<div style="vertical-align:left; width: 100px;height: 600px;float:left;">
						<input type="button" onclick="addContentText()" class="userOperBtn" value="添加文字"></input> 
						<input type="button" onclick="addContentImgPanel()" class="userOperBtn" value="添加图片"></input> 
						<input type="button" onclick="addContentGoodsPanel()" class="userOperBtn" value="添加商品"></input>
				<div> 
               </td>
			</tr>
			<tr>
...
<div id="uploadImgDiv" class="showbox"> 
        <form id="addItemImgSubjectForm" name="addItemImgSubjectForm" enctype="multipart/form-data" method="post" action="${rc.getContextPath()}/subject/saveImg">
			<div>请选择图片: <input type="file"  style="width:400px"  name="item_pic" id="item_pic"/> </div>
...
<div id="findGoodsDiv" class="showbox"> 
			...
			<div id="item_goods_error_msg"></div>
			<div class="btnsdiv">
			<input value="确定" class="userOperBtn panelBtn" type="button" id="goodsImgBut" onclick="addContentGoods()"/> <input value="取消" class="userOperBtn panelBtn" type="button"  onclick="showPanelClose()"/> 
			</div>
</div>
</body>
...
$(function(){
	 ...
	 for(var i=0;i<texts.length;i++){
	    $(texts[i]).height($(texts[i])[0].scrollHeight);
	 }
...
function addContentText(){
	  ...
	  +'<input type="hidden" name="subjectItem" class="subjectItem" value="" />'
	  +'<textarea class="content_text" style="overflow-y: hidden;height:30px" onpropertychange="this.style.height = this.scrollHeight+ \'px\';" oninput="this.style.height = this.scrollHeight + \'px\';">输入文字</textarea>'
	  +'<select  name="urlType" id="status" style="width:120px;margin-left:8px;margin-right:8px;float: left;">'
                    ...
                    success:function(data){
                              ...
                              var realHeight;//真实的高度
                              	$("<img/>").attr("src", imgsrc).load(function() {
									  ...
									  var sel=$('<select  name="urlType" id="status" style="width:120px;margin-left:8px;margin-right:8px;float: left;">'
											+'<option value="URL">URL</option><option value="卖场">卖场</option><option value="卖场集">卖场集</option>'
											+'<option value="商品分类">商品分类</option><option value="品牌">品牌</option><option value="话题">话题</option>'
											+'<option value="秀">秀</option><option value="商品">商品</option><option value="专题">专题</option></select><input name="subjectUrl" value="" style="height:20px;float: left;"/>');
								     sel.insertAfter(img);
...
function getTimesByOrderSeq(){
   ...
   $("#timemsg").html("");
	    $.ajax({
				type : "GET",
				async: false,
				url : "${rc.getContextPath()}/subject/getTimesByOrderSeq?subjectId="+subjectId+"&orderSeq=" + orderSeq +"&displayStytle="+displayStytle+"&format=json",
	            dataType: "text",
				success : function(data, textStatus) {
...
function checkTimesByOrderSeq(){
   ...
   var isSuccess=false;
	    $.ajax({
				type : "GET",
				async: false,
				url : "${rc.getContextPath()}/subject/checkTimesByOrderSeq?subjectId="+subjectId+"&orderSeq=" + orderSeq +"&displayStytle="+displayStytle+"&beginTime="+beginTime+"&endTime="+endTime+"&format=json",
	            dataType: "text",
				success : function(data, textStatus) {
                            if(data!=0){
