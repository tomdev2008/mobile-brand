...
<form id="mainForm" name="mainForm" action="${rc.getContextPath()}/subject/commentlist?page.pageNo=1" method="GET">
	...
	<div class="adminUp_wrap">
		...
		<fieldset class="clearfix adminSearch">
                <div class="wapbt" style="border-bottom:0px;" align="left">
				      ...
				      <label>专题ID：
				          <input name="subjectId" type="text" id="subjectId" value="<#if subjectId??>${subjectId}</#if>" size="20"  />
				          &nbsp;
				      </label>
				      <label>评论时间：
	...
	<div class="adminContent clearfix">
		<div class="adminContent clearfix">
			<table width="100%" class="table_bg01 table_hg01">
				...
				<tbody>
				 	<#if (subjectCommentList?? && subjectCommentList?size > 0)> 
				     <#list subjectCommentList as menu>
				      <tr>
				        <td>
						...
						<td>${menu.content!''}</td>
						<td><a href="#" onclick="openSubjectInfo(${menu.subjectId!''})">${menu.subjectName!''}</a></td>
					 ...
					 <#elseif  menu.deleteFlag??&&menu.deleteFlag == 3>后台删除
						...
						<td>
						  ...
						  |<a href="javascript:void(0);" onclick="showComfigReport('${menu.id}','1')" title="删除">删除</a> 
						   | <a href="javascript:void(0);" onclick="addComment('${menu.subjectId!''}','${menu.userId!''}')" title="回复">回复</a>
...
<script type="text/javascript">
 ...
 function openSubjectInfo(subjectId){
	...
	function viewComment(commentId) {
		var pageUrl = "${rc.getContextPath()}/subject/commentInfo?commentId="+commentId;
	    openUrl(pageUrl);
	}
...
function deleteBatch(){
	...
	if(ids != ""){
		if(confirm("确实要删除吗？")== true){
		    $.ajax({
				type : "GET",
				url : "${rc.getContextPath()}/subject/deleteBatchComment?ids=" + ids + "&format=json",
	            dataType: "text",
				success : function(data, textStatus) {
				   if (isNaN(data)) {
						var objs =  $.parseJSON(data);
 ...
 //添加评论
	...
	function addSubjectComment(){
		...
		$.ajax({
			type : "GET",
			url : "${rc.getContextPath()}/subject/addComment?subjectId="+subjectId+"&userId="+userId+"&content="+content+"&commentedUserId="+commentedUserId+"&format=json",
            dataType: "text",
			success : function(data, textStatus) {
			   if (isNaN(data)) {
					var objs = $.parseJSON(data);
