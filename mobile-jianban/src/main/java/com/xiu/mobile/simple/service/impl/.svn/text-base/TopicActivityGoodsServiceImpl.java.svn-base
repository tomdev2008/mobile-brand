...
import com.xiu.mobile.simple.common.constants.OrderRuleConstant;
import com.xiu.mobile.simple.common.constants.XiuConstant;
import com.xiu.mobile.simple.common.util.ComparatorDiscountAsc;
import com.xiu.mobile.simple.common.util.ComparatorDiscountDesc;
import com.xiu.mobile.simple.common.util.ComparatorPriceAsc;
import com.xiu.mobile.simple.common.util.ComparatorPriceDesc;
...
import com.xiu.mobile.simple.dao.TopicActivityGoodsDao;
...
public class TopicActivityGoodsServiceImpl implements ITopicActivityGoodsService {
	...
	public ActivityGoodsBo queryTopicActivityGoodsList(
		...
		if (!"true".equals(isHotSale)) {
			List<GoodsVo> gListAll = topicActivityDao.queryTopicActivityGoodsList(paramMap);
			...
			if (lstGoodsVoCache.size() > 0) {
				...
				for (GoodsVo vo : lstGoodsVoFinal) {
					...
					if (StringUtils.isNotEmpty(vo.getGoodsImg())) {
						...
						String imgUrl = "http://" + arr[index%3] + ".xiustatic.com";
	...
	private List<GoodsVo> getGoodsListByCategory(
		...
		for (TopicActivityGoodsVo vo : goodsVoListByCategory) {
			result.add((GoodsVo) goodsMap.get(vo.getGoodSn()));
		}
	 ...
	 * @throws InterfaceHandleException
	 */
	private GoodsVo assembleGoodsItem(
			Product product, List<ItemSettleResultDO> resultDOList) {
...
//		throw new InterfaceHandleException(GlobalConstants.RET_CODE_SYS_ERROR, "获取商不到商品价格");
	...
	public List<GoodsVo> sort(List<GoodsVo> src, List<GoodsVo> dest) {
		...
		logger.info("before sort:src="+src);
		logger.info("before sort:dest="+dest);
