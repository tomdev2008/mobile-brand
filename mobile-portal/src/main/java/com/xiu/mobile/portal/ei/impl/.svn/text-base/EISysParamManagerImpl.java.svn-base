...
public class EISysParamManagerImpl implements EISysParamManager {
	...
	public CspResult getAllRegionParamInfo() {
		CspResult result = null;
		try {
			result = this.sysParamFacade.getAllRegionParamInfo(); 
		} catch(Exception e){
			logger.error("{}.getAllRegionParamInfo 调用CSP接口异常 | message={}",
					new Object[]{sysParamFacade.getClass(),e.getMessage()});
			throw ExceptionFactory.buildEIRuntimeException(ErrConstants.EIErrorCode.EI_CSP_ALL_REGION_FAILED_ERR, e);
		}
		if(!"1".equals(result.getSuccess())){
			logger.error("{}.getAllRegionParamInfo 调用CSP接口失败 | errCode={} | errMessage={}",
					new Object[]{sysParamFacade.getClass(),result.getErrorCode(),result.getErrorMessage()});
			throw ExceptionFactory.buildEIBusinessException(ErrConstants.EIErrorCode.EI_CSP_BIZ_ERR, result.getErrorCode(), result.getErrorMessage());
		}
		return result;
	}
