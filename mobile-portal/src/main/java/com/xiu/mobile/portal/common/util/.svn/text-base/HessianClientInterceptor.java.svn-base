...
public class HessianClientInterceptor extends UrlBasedRemoteAccessor implements MethodInterceptor {
    ...
    public Object handleInvoke(MethodInvocation invocation) throws Throwable {
		...
		try {
			...
			if (ex.getTargetException() instanceof HessianRuntimeException) {
				...
				throw convertHessianException(rootCause);
				...
				throw convertHessianException(utex.getUndeclaredThrowable());
			}
     ...
     * @return the RemoteAccessException to throw
     */
    protected Exception convertHessianException(Throwable ex) {
        if (ex instanceof ConnectException) {
            return new RemoteConnectFailureException("远程调用Hessian连接出错：Cannot connect to Hessian remote service at [" + getServiceUrl() + "]", ex);
        }else {
            return new Exception("远程调用Hessian异常： Hessian remote service exception at [" + getServiceUrl() + "]", ex);
        }
    }
