...
public class EIPromotionManagerImpl implements EIPromotionManager {
	...
	@Autowired
	private ProdSettlementHessianService prodSettlementService;
	...
	public Result sendCardByDeed(SendCardIntegDO sendCardIntegDO) {
		...
		try {
			...
			LOGGER.error("优惠券发放sendCardByDeed异常：e="+e,e);
			throw ExceptionFactory.buildEIRuntimeException(ErrConstants.EIErrorCode.EI_PROMOTION_GENERAL_ERR, e);
		}
		
		if (!result.isSuccess()) {
			String errorMessage = produceErrorMes(result);
			LOGGER.error("invoke remote interface [Promotion] sendCardIntegService.sendCardByDeed error.");
	...
	public String activeCardBody(CardInputParamDO cardInput) {
		...
		try {
			...
			LOGGER.error("优惠券激活activeCardBody异常：e"+e);
		...
		if(cardOut == null || StringUtil.isEmpty(cardOut.getStatus())) {
			...
			throw ExceptionFactory.buildEIBusinessException(
					ErrConstants.EIErrorCode.EI_PROMOTION_BIZ_ACB_ERR, "", null);
		}
...
//			result = itemListSettleService.itemListSettleSrevice(itemSettleResults);
			result = prodSettlementService.prodListSettle(itemSettleResults);
		} catch (Exception e) {
		...
		if (!result.isSuccess()) {
			...
			LOGGER.error("invoke remote interface [Promotion] prodSettlementService.prodListSettle error.");
			LOGGER.error("ErrorCode:" + result.getResultCode());
			LOGGER.error("ErrorMessage:" + errorMessage);
	...
	public Result itemSettleForXmlRpc(ItemSettleResultDO param) {
		...
		Result result = null;
		try {
			param.setTerminalUser("1");//描述：终端用户(1 手机用户 2 电脑用户,多个用逗号分开)，默认值2。 其它参数不变
//			result = itemSettleService.itemSettleForXmlRpc(param);
			//调用结算系统的新接口
			result = prodSettlementService.prodSettle(param);
		} catch (Exception e) {
		...
		if (!result.isSuccess()) {
			...
			LOGGER.error("invoke remote interface [Promotion] prodSettlementService.prodSettle");
			LOGGER.error("ErrorCode:" + result.getResultCode());
			LOGGER.error("ErrorMessage:" + errorMessage);
	...
	@Override
	public ProdSimpleSettleDO prodSimpleSettle(String goodsSn,Long basePrice) {
		...
		try {
			...
			priceSettlementDO.setBasePrice(basePrice);
			prodSimpleSettleDO.setPrice(priceSettlementDO);
			// 获取商品报关、物流信息
			result = prodSettlementService.prodSimpleSettle(prodSimpleSettleDO);
		} catch (Exception e) {
			LOGGER.error("获取商品报关、物流信息prodSimpleSettle异常：e"+e,e);
			throw ExceptionFactory.buildEIRuntimeException(ErrConstants.EIErrorCode.EI_PROMOTION_GENERAL_ERR, e);
		}
		...
		if (!result.isSuccess()) {
			...
			LOGGER.error("invoke remote interface [Promotion] prodSettlementService.prodSimpleSettle");
			LOGGER.error("ErrorCode:" + result.getResultCode());
			LOGGER.error("ErrorMessage:" + errorMessage);
			
			throw ExceptionFactory.buildEIBusinessException(ErrConstants.EIErrorCode.EI_PROMOTION_BIZ_ERR, 
					result.getResultCode(), errorMessage);
		}
		
		ProdSimpleSettleDO prodSimpleSettleDO = (ProdSimpleSettleDO) result.getDefaultModel();
		return prodSimpleSettleDO;
	}
}
